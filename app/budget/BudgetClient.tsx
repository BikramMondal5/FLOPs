...
export default function BudgetClient({ initialData, userName }: BudgetClientProps) {
  ...
  const handleCreateBudget = async (name: string, total: number, category: string) => {
    try {
      const now = new Date();
  ...
  return (
    ...
    >
      ...
      {/* Workspace with top padding for fixed navbar */}
              ...
              {budgets.map((eb) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    <span className="text-2xl">{eb.budget.icon === "utensils" ? "🍔" : "⚡"}</span>
                    <div>
            ...
            <div className="flex flex-col gap-3">
              ...
              {alerts.length === 0 && (
                <div className="py-4 text-center text-xs text-zinc-400">
                  No utilization alerts triggered. Trackers are running on track.
                </div>
              )}
            </div>
          </div>
      ...
      <AnimatePresence>
        {createModalOpen && (
          <CreateBudgetModal
            ...
            onCreate={(name, total) => handleCreateBudget(name, total, "Food & Dining")}
          />
        )}
      </AnimatePresence>
    </div>
